...
update_os_components_deb()
{
	...
	# icu-devtools is for autoinstaller: libicu66 on Ubuntu 20.04, libicu60 on Ubuntu 18.04, libicu67 on Debian 11, libicu63 on Debian 10, libicu57 on Debian 9, libicu52 on Debian 8
	for pkg in 'ca-certificates' 'gnupg' 'icu-devtools'; do
		dpkg -s "$pkg" > /dev/null 2>&1 || missing_packages="$missing_packages $pkg"
	done

	if [ "$os_name" = "Debian" -a "${os_version%%.*}" = "9" ]; then
		if [ -n "$source" -a -z "${source##https://*}" ] && ! is_package_installed_deb "apt-transport-https"; then
			missing_packages="$missing_packages apt-transport-https"
		fi
	fi
