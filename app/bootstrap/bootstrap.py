...
import os
import sys
from app.bootstrap.setup_check import (
    init_tmux,
    run_updates,
    tmux_activated,
    venv_activated,
    ...
    if args.command == "run":
        if args.run == "assistant":
            if tmux_activated() and venv_activated():
                ...
                print(
                    "Either TMUX of VENV are not activated!\n"
                    "See printout below:\n"
            ...
            args.run == "legacy"
            and getattr(args, "legacy", False) == "assistant"
        ):
            os.execv(
                "/bin/bash",
                ["bash", f"{ROOT_DIR}/assistant.legacy.sh"] + sys.argv[1:],
            )
            sys.exit(0)
